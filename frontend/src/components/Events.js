...
import { isAfter, isBefore, parse, add, sub, compareAsc, isValid } from 'date-fns'
  ...
  const getActiveEvents = (isForCalendar = false) => {
    ...
    if (!isForCalendar) {
      if (searchPhrase.length > 0) {
        arr = arr.filter((event) => event.title.toLowerCase().match(searchPhrase.toLowerCase()))
      }

      if (isValid(dateFrom)) {
        arr = arr.filter((event) => isAfter(event.date_iso, sub(dateFrom, { days: 1 })))
      }
      if (isValid(dateTo)) {
        arr = arr.filter((event) => isBefore(event.date_iso, add(dateTo, { days: 1 })))
      }
    }
    arr.sort((a, b) => compareAsc(a.date_iso, b.date_iso))

    if (viewActive !== 'All') {
      arr = arr.filter((event) => event.is_active === (viewActive === 'Active'))
    }
    if (sortReversed) {
      return arr.reverse()
    }
        ...
        <div
          ...
          <div className='p-6'>
            <div className='flex flex-row'>
              <div className='object-center mr-4'>{renderEventButtons(event)}</div>
              <h2 className='tracking-widest text-xs title-font font-medium text-gray-400 mb-1' id='dateAndPlace'>
                {event.date} at <a href={`/places/${event.place}`}>{event.place_name}</a>
              </h2>
            </div>
    ...
    return (
      <div>
        ...
        <div className={`menu-buttons my-2 ${gridDisplay ? '' : 'invisible'}`}>
          <button className={`flex items-center ${isValid(dateFrom) || isValid(dateTo) ? 'toggle' : ''}`}>
            ...
            <div className='relative'>
              <input
                ...
                onChange={(e) => setDateFrom(parse(e.target.value, 'yyyy-MM-dd', new Date()))}
              />
            </div>
            ...
            <div className='relative'>
              <input
                ...
                onChange={(e) => setDateTo(parse(e.target.value, 'yyyy-MM-dd', new Date()))}
              />
            </div>
          </button>
          <button onClick={() => setSortReversed(!sortReversed)} id='sortButton'>
            {sortReversed ? '⬆' : '⬇'}
          </button>
        ...
        <div className='card p-3 mx-5'>
          {gridDisplay ? (
            <section className='text-gray-600 dark:text-grey-200 body-font'>
              <div className='container px-5 py-5 mx-auto flex flex-wrap -m-4'>{renderGridItems()}</div>
            </section>
          ) : (
            <div>
              {<MyCalendar events={getActiveEventsForCalendarView()} cancelEvent={(event) => changeActive(event)} />}
            </div>
          )}
        </div>
      </div>
