...
const PlacePage = () => {
  ...
  const renderGridItems = () => {
    ...
    return activeEvents.map((event) => (
      <div className='p-4 md:w-1/3 max-w-xl max-h-md' id='eventCard' key={event.id}>
        <div
          className={`h-full ${
      ...
      <div className='container px-5 pt-12 pb-24 mx-auto flex flex-col'>
        <div className='lg:w-5/6 mx-auto'>
          ...
          <div className={`rounded-lg overflow-hidden ${imgClip ? 'h-64' : ''}`}>
            <img alt='content' className='object-cover object-center h-full w-full' src={place.picture} />
          </div>
